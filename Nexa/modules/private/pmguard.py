...
from Nexa.database.pmpermitdb import get_approved_users, pm_guard
import Nexa.database.pmpermitdb as Nexa
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Nexa.set_block_message(Nexa.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Nexa.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Nexa.get_pm_settings()
    await Nexa.allow_user(chat_id)
